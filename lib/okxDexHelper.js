...
async function getQuote(
    ...
    try {
        console.log('Getting quote data from OKX API...');
        ...
        console.log('Quote API Request Parameters:', JSON.stringify(params, null, 2));
        
        const result = await sendGetRequest(path, params);
        return result[0];
    } catch (error) {
        console.error('Failed to get quote data:', error.message);
        throw error;
    }
}
...
setTimeout(() => {
    /// v6 quote
    sendGetRequest('/api/v6/dex/aggregator/quote', {
        chainIndex: 1,
