...
function preHash(timestamp, method, request_path, params) {
    ...
    if (method === 'GET' && params) {
        ...
        query_string = '?' + buildQuery(params);
    }
...
async function getGasLimit(
    ...
    try {
        ...
        const results = await sendPostRequest(path, params);
...
// ID DEX 名称 描述 对合约调用友好?
// 1 Uniswap V2最经典的 AMM DEX，使用恒定乘积公式 x*y=k✅ 是
// 2 Uniswap V3V2 的升级版，支持集中流动性✅ 是
// 3 SushiSwap Uniswap V2 的分叉，机制类似✅ 是
// 4 Curve         专门为稳定币优化的 DEX✅ 是
// 5 Balancer V2  支持多种资产池✅ 是
// 6 Balancer V1  Balancer 的旧版本❌ 不友好！
// 7 dYdX  去中心化衍生品交易所✅ 通常可以
// 8 0x Protocol  链下订单簿 + 链上结算✅ 是
/**
